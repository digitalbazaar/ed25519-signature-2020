...
export class Ed25519Signature2020 extends LinkedDataSignature {
   ...
   *   canonize algorithm.
   * @param {boolean} [options.compactProof=false] - Recommendation to
   *   jsonld-signature on whether to compactProof. This is defined per suite,
   *   and the expectation is that the doc being verified conforms to the
   *   suite's context.
   */
  constructor({
    ...
    super({
      ...
      signer, key, proof, date, useNativeCanonize, compactProof
    });
   ...
   * @param {Uint8Array} options.verifyData - Data to be signed.
  ...
  async sign({verifyData, proof}) {
    ...
    proof.proofValue = {'@value': `z${base58btc.encode(signatureBytes)}`,
